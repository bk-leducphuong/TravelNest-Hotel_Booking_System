...
class PayoutRepository {
  ...
  async findByIdWithItems(payoutId, options = {}) {
    return await Payouts.findOne({
      where: { id: payoutId },
      include: [...this.defaultInclude(), { model: PayoutItems, as: 'items', required: false }],
      ...options,
    });
  }

  async findItems(payoutId, options = {}) {
    return await PayoutItems.findAll({
      where: { payout_id: payoutId },
      ...options,
    });
  }

  async sumByStatus(hotelId, options = {}) {

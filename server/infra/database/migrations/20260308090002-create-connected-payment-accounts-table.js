...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await createTableIfMissing(queryInterface, 'connected_payment_accounts', {
        id: {
          type: Sequelize.UUID,
          allowNull: false,
          primaryKey: true,
    ...
    await addIndexIfMissing(queryInterface, 'connected_payment_accounts', ['user_id'], {
      name: 'idx_connected_payment_accounts_user_id',
    });

    await addIndexIfMissing(queryInterface, 'connected_payment_accounts', ['hotel_id'], {
    ...
    await queryInterface.sequelize.query(`
      ...
      WHERE connect_account_id IS NOT NULL
        ...
        AND NOT EXISTS (
          SELECT 1
          FROM connected_payment_accounts cpa
          WHERE cpa.provider = 'stripe'
            AND cpa.provider_account_id = users.connect_account_id
        )
    `);
  },

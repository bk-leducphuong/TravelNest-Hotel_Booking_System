...
describe('HotelRepository', () => {
  ...
  describe('checkRoomAvailability', () => {
    it('should check availability for multiple rooms', async () => {
      ...
      mockSequelize.query.mockResolvedValue(mockAvailability);

      // Act
      const result = await hotelRepository.checkRoomAvailability(
        1,
        [1, 2],
        '2024-01-01',
        '2024-01-05',
      ...
      // Assert
      expect(mockSequelize.query).toHaveBeenCalledWith(
        expect.stringContaining('MIN(ri.total_rooms - ri.booked_rooms)'),
        {
          replacements: [1, [1, 2], '2024-01-01', '2024-01-05', 4],
          type: 'SELECT',
        }
      );
    ...
    it('should return empty array when rooms not available', async () => {
      // Arrange
      mockSequelize.query.mockResolvedValue([]);
    ...
    it('should handle single room check', async () => {
      // Arrange
      const mockAvailability = [{ room_id: 1, available_rooms: 10 }];
      mockSequelize.query.mockResolvedValue(mockAvailability);
      ...
      expect(mockSequelize.query).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({
          replacements: [1, [1], '2024-01-01', '2024-01-05', 4],
        })
      );
      expect(result).toEqual(mockAvailability);
    });
